...
private:
    ...
    bool appeal_information_search(appeal_info,appeal_info);//查找
    void appeal_information_search(partner *);              //通过appealID查找信息
    void appeal_information_delete(appeal_info);            //删除诉求
...
signals:
    ...
    void touch_no_happen_to_jsonparser(partner,appeal_info);//下一个
...
public slots:
    ...
    void get_appeal_cancel_from_jsonparser(appeal_info);    //诉求取消处理函数
